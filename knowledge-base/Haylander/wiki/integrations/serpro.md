...
- **Alias operacional:** enquanto `PGFN_CONSULTAR` for alias de `DIVIDAATIVA24`, não há separação real de comportamento entre as duas rotas.
...
1. Subir `PGFN_CLIENT_ID` e `PGFN_CLIENT_SECRET` reais no ambiente de produção antes de ativar teste com cliente.
2. Padronizar resolução multi-empresa em todas as sincronizações (priorizar `cnpj_ativo` e vínculo em `lead_empresa`).
3. Adicionar resumo visual de PGFN no Admin (inscrições, valores, situação, última verificação) para reduzir interpretação manual.
