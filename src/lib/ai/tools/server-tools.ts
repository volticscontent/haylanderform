...
export async function callAttendant(phone: string, reason: string = 'Solicitação do cliente'): Promise<string> {
  const client = await pool.connect();
  try {
    // Registra o pedido de atendimento no banco de dados
    await client.query(
      `UPDATE leads SET needs_attendant = true, attendant_requested_at = NOW() WHERE telefone = $1`,
      [phone]
    );
...
export async function searchServices(query: string): Promise<string> {
  const client = await pool.connect();
  try {
    // Busca serviços similares na tabela
    const res = await client.query(
      'SELECT nome as name, descricao as description, price_info FROM services WHERE active = true AND nome ILIKE $1',
      [`%${query}%`]
    );
    if (res.rows.length > 0) {
    ...
    const all = await client.query('SELECT nome as name, descricao as description, price_info FROM services WHERE active = true');
...
export async function interpreter(
  ...
  try {
    ...
    // 2. Handle Actions
    if (action === 'post') {
      const truncatedText = text.substring(0, 1000); // Prevenção de Abuso V5
      ...
      if (embedding && embedding.length > 0) {
        try {
          ...
          await client.query(
            ...
            [phone, truncatedText, category, vectorStr]
          );
        } catch {
