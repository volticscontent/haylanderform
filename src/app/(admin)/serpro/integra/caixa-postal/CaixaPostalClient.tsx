...
export default function CaixaPostalClient({ mensagens }: { mensagens: CaixaPostalMsg[] }) {
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            <Mail className="w-6 h-6 text-purple-600 dark:text-orange-500" /> Caixa Postal
          </h1>
        ...
        <button
          ...
          disabled={pending}
          className="flex items-center gap-2 px-4 py-2 text-sm bg-purple-600 hover:bg-purple-700 dark:bg-orange-500 dark:hover:bg-orange-600 text-white rounded-lg disabled:opacity-50 transition-colors"
        >
      ...
      {syncMsg && (
        <div className="bg-purple-50 dark:bg-orange-950/20 border border-purple-200 dark:border-orange-800 rounded-lg px-4 py-2 text-sm text-purple-700 dark:text-orange-400">
          {syncMsg}
        </div>
      )}
      ...
      {mensagens.length === 0 ? (
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 divide-y divide-zinc-100 dark:divide-zinc-800">
          {mensagens.map(m => (
            <div
              key={m.id}
              className={`px-4 py-4 ${!m.lida ? 'bg-purple-50/30 dark:bg-orange-950/10' : ''}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  {m.lida
                    ? <MailOpen className="w-4 h-4 text-zinc-400 mt-0.5 shrink-0" />
                    : <Mail className="w-4 h-4 text-purple-600 dark:text-orange-500 mt-0.5 shrink-0" />}
