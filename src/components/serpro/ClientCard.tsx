...
export default function ClientCard({ client, className = '', onSelectCnpj }: ClientCardProps) {
  ...
  return (
    <>
      <div className={`group relative bg-white/70 dark:bg-zinc-900/50 backdrop-blur-xl rounded-2xl border border-zinc-200/60 dark:border-zinc-800/60 p-5 shadow-sm hover:shadow-2xl hover:shadow-purple-500/5 dark:hover:shadow-orange-900/20 transition-all duration-500 hover:-translate-y-1 ${className}`}>
            ...
            <div className="flex justify-between items-start">
              <h3 className="text-[15px] font-bold text-zinc-900 dark:text-zinc-50 line-clamp-1 mr-2 tracking-tight transition-colors group-hover:text-blue-600 dark:group-hover:text-blue-400" title={client.nome}>
                ...
                {client.data_ultima_consulta && (
                  <span className="ml-1 pl-1 border px-1.5 rounded-full border border-zinc-200 dark:border-zinc-700 flex items-center gap-0.5 text-emerald-600 dark:text-emerald-400">
        ...
        <div className="mt-4 pt-3 border-t border-zinc-200/50 dark:border-zinc-800/50 flex justify-between items-center relative z-10">
          {onSelectCnpj ? (
            <button
              ...
              className="text-xs flex items-center gap-1.5 text-black dark:text-white font-bold transition-all px-2.5 py-1.5 rounded-md hover:bg-orange-100 dark:hover:bg-orange-100/10 active:scale-95"
            >
          ...
          <button
            ...
            className="text-xs flex items-center gap-1.5 text-indigo-600 dark:text-orange-400 font-bold transition-all px-2.5 py-1.5 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-500/10 active:scale-95"
          >
