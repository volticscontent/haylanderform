    ...
    if (message.mediaType === 'document' && message.mediaUrl) {
      ...
      return (
        <div className="flex flex-col gap-1 min-w-[220px]">
          <div className={`flex items-center gap-3 p-3 rounded-lg border ${isMe
            ? 'bg-emerald-100/50 dark:bg-emerald-900/30 border-emerald-200/50 dark:border-emerald-800/50'
            : 'bg-zinc-100 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700'
            }`}>
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${isPdf
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`p-2 rounded-full transition-colors shrink-0 ${isMe
  ...
  return (
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} ${marginBottom} group select-none`}>
      ...
      >
        ...
        <div className={`text-[10px] flex items-center justify-end gap-1 mt-1 select-none min-h-[16px] ${isMe ? 'text-emerald-900/40 dark:text-emerald-100/50' : 'text-zinc-400 dark:text-zinc-500'
          }`}>
          <span>
